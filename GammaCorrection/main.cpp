...
#define PROJECT_NAME "GammaCorrection"

void framebuffer_size_callback(GLFWwindow* window, int width, int height);
void mouse_callback(GLFWwindow* window, double xpos, double ypos);
void processInput(GLFWwindow* window);
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset);
unsigned int loadTexture(const std::filesystem::path&, bool gammaCorrection);
...
int main()
{
    ...
    // Texture 
    //-------------------------------------------------------------------------
    unsigned int diffuseMap = loadTexture(std::filesystem::current_path() / "../../../../resource/textures/container2.png", true);
    unsigned int specularMap = loadTexture(std::filesystem::current_path() / "../../../../resource/textures/container2_specular.png", true);
    unsigned int woodFloor = loadTexture(std::filesystem::current_path() / "../../../../resource/textures/wood.png", false);
    unsigned int woodFloorGammaCorrected = loadTexture(std::filesystem::current_path() / "../../../../resource/textures/wood.png", true);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        */
        // floor
        {
            ...
            glUniform3fv(glGetUniformLocation(floorShader, "lightPositions"), 4, &pointLightPositions[0][0]);
            glUniform3fv(glGetUniformLocation(floorShader, "lightColors"), 4, &lightColors[0][0]);
            floorShader.set("viewPos", camera.Position);
            floorShader.set("gamma", gammaEnabled);

            glBindVertexArray(floor_vao);
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, gammaEnabled? woodFloorGammaCorrected : woodFloor);
            glDrawArrays(GL_TRIANGLES, 0, 6);
            std::cout << std::boolalpha << gammaEnabled << '\n';
        }
        // lighting source
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                lightSrcShader.set("cubeColor", lightColors[i]);
                glDrawArrays(GL_TRIANGLES, 0, 36);
            }
...
unsigned int loadTexture(const std::filesystem::path& path, bool gammaCorrection)
{
